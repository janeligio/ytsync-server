...
const { randomId, generateAlias, parseURL } = require('./src/utility/utility');
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        log(
            `Socket#${socket.id} disconnected, alias:${Aliases.get(socket.id)}`
        );
    ...
    socket.on(Events.create_room, (clientAlias, callback) => {
        ...
        if (!roomExists) {
            ...
            if (YTsyncRoom) {
                ...
                socket.emit(Events.receive_room_state, {
                    chatHistory,
                    queue,
                    currentVideo,
                });
